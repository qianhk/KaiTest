...
import com.njnu.kai.test.danmaku.bitmap.DeviceUtils;
import com.njnu.kai.test.danmaku.danmaku.model.*;
import com.njnu.kai.test.danmaku.danmaku.model.android.AndroidDisplayer;
...
public class DrawHandler extends Handler {
    ...
    public static final int NOTIFY_STOP = 100;
    ...
    private void updateInNewThread() {
        ...
        mThread = new UpdateThread("Danmaku update") {
            @Override
            public void run() {
                ...
                while (!isQuited() && !quitFlag) {
                    long startMS = System.currentTimeMillis();
                    dTime = startMS - lastTime;
                    long diffTime = mFrameUpdateRate - dTime;
                    if (diffTime > 1) {
                        SystemClock.sleep(Math.min(5, diffTime));
                        continue;
                    }
                    lastTime = startMS;
                    long d = syncTimer(startMS);
...
//            Log.e("DrawHandler", time+"|d:" + d  + "RemaingTime:" + mRemainingTime + ",gapTime:" + gapTime + ",rtim:" + mRenderingState.consumingTime + ",average:" + averageTime);
    ...
    private void initRenderingConfigs() {
        ...
        mCordonTime = Math.max(60, (long) (averageFrameConsumingTime * 2.5f));
        mCordonTime2 = mCordonTime * 2;
        mFrameUpdateRate = Math.max(30, averageFrameConsumingTime / 15 * 15);
    ...
    public void seekTo(Long ms) {
        ...
        if (isStop()) {
            sendEmptyMessageDelayed(NOTIFY_STOP, 200);
        }
    ...
    public void resume() {
        ...
        if (hasMessages(NOTIFY_STOP)) {
            removeMessages(NOTIFY_STOP);
        }
    }
