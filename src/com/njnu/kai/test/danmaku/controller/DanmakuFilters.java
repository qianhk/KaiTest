...
import com.njnu.kai.test.danmaku.danmaku.model.IDanmakuIterator;
import com.njnu.kai.test.danmaku.danmaku.model.IDanmakus;
...
public class DanmakuFilters {
    ...
    public static class QuantityDanmakuFilter extends BaseDanmakuFilter<Integer> {
        ...
        public synchronized boolean filter(BaseDanmaku danmaku, int orderInScreen,
            ...
            if (mMaximumSize <= 0 || danmaku.getType() != BaseDanmaku.TYPE_SCROLL_RL || danmaku.isLive) {
                return false;
            }
     ...
     * @author qhk
     */
    public static class QuantityFixDanmakuFilter extends BaseDanmakuFilter<Integer> {

        private static final String TAG = "QuantityFixDanmakuFilter";

        protected BaseDanmaku mLastSkipped = null;
        protected BaseDanmaku mLastSkippedBottom = null;
        ...
        public synchronized boolean filter(BaseDanmaku danmaku, int orderInScreen,
            ...
            if (type != BaseDanmaku.TYPE_FIX_TOP && type != BaseDanmaku.TYPE_FIX_BOTTOM) {
                return false;
            }
            ...
            if (type == BaseDanmaku.TYPE_FIX_TOP) {
                if (mLastSkipped == null || danmaku.isShown() || (danmaku.time - mLastSkipped.time > 980)) {
                    mLastSkipped = danmaku;
                    return false;
                }
                return true;
            } else {
                if (mLastSkippedBottom == null || danmaku.isShown() || (danmaku.time - mLastSkippedBottom.time > 980)) {
                    mLastSkippedBottom = danmaku;
                    return false;
                }
                return true;
            }
    ...
    public final static String TAG_QUANTITY_FIX_DANMAKU_FILTER = "1111_Filter";
    ...
    public IDanmakuFilter<?> getNotRegister(String tag) {
        return filters.get(tag);
    }
    ...
    public IDanmakuFilter<?> registerFilter(String tag) {
        ...
        if (filter == null) {
            ...
            } else if (TAG_QUANTITY_FIX_DANMAKU_FILTER.equals(tag)) {
                filter = new QuantityFixDanmakuFilter();
            }
            // add more filter
        }
        if (filter == null) {
